...
export function createUpdater(deps = {}) {
  ...
  return {
    ...
    start(version, trigger = 'manual') {
      ...
      // Install from wherever the offered versions actually came from. The
      // configured registry is the wrong source precisely when it matters: a
      // mirror only ever answers because the configured URL was unreachable, and
      // re-asking that URL for the version on screen reports it as missing. Read
      // at spawn time, so the most recent successful check decides.
      const served = deps.servedRegistry !== undefined ? deps.servedRegistry() : undefined
      const source = served ?? deps.registry
      const registryArgs = source === undefined ? [] : ['--registry', normalizeRegistry(source)]
