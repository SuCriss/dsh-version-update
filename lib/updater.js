...
export function createUpdater(deps = {}) {
   ...
   * — by settle() — whether the run succeeded, failed, or was killed at the
   * hard ceiling.
  ...
  const settle = (state, error) => {
    ...
    // Release the machine-wide lock: npm has settled either way, so the tree
    // is no longer being written by this run.
    machineLock?.release()
  ...
  return {
    ...
    start(version, trigger = 'manual') {
      ...
      // once. Acquired here, before anything else claims the slot, and
      // released by settle(). A refusal is a clean 409-style error, not a
      ...
      hardTimer = setTimeout(() => {
        ...
        settle('failed', 'install exceeded the hard time limit')
