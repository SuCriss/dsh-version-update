...
import { createSnapshotAsync, defaultSnapshotsDir, listSnapshots, removeSnapshot, restoreSnapshotAsync } from './snapshot.js'
...
export function apply(ctx, config = {}) {
  ...
  const snapshotOps = installDir === undefined
    ...
    : {
       ...
       * @returns {Promise<{ ok: boolean; error?: string }>} the outcome.
       */
      remove: async (version) => await withUpdateLock(() => (removeSnapshot(snapshotsDir, version)
        ? { ok: true }
