/**
 * Version facts and the registry read behind the panel: discover the running
 * dsh installation, fetch the npm packument, rank versions, and — the heart of
 * the policy engine — resolve a tracking rule into one concrete install
 * target.
 *
...
import { DSH_PACKAGE, LINE_PATTERN } from './protocol.js'
...
/**
 * Registries tried in order when the primary one is unreachable: the public
 * registry works everywhere but is often blocked or flaky behind slow links,
 * so a failure there falls back to a regional mirror instead of degrading the
 * panel to a local-only view. The mirror list is deliberately small — each
 * entry is one more network call on an already-failing path.
 */
export const FALLBACK_REGISTRIES = ['https://registry.npmmirror.com']
...
/**
 * Whether a STABLE version satisfies a tracking line (`^x.y.z` or `~x.y.z`).
 *
 * Pre-release versions never satisfy a line: a follower of `^0.4.0` wants the
 * newest stable of that minor, not whatever `-rc` happens to be out. The
 * range's own lower bound is deliberately not re-checked here — callers rank
 * the matching set and compare against what is installed, so the bound adds
 * nothing but code.
 * @param {string} version - the published version.
 * @param {string} range - the tracking line text.
 * @returns {boolean} true when the version belongs to the line.
 */
export function matchesLine(version, range) {
  const match = LINE_PATTERN.exec(range)
  if (match === null) return false
  const parsed = parseVersion(version)
  if (parsed === undefined || parsed.pre.length > 0) return false
  const [major, minor, patch] = parsed.core
  if (major !== Number(match[2])) return false
  if (match[1] === '~') return minor === Number(match[3]) && patch >= Number(match[4])
  // Caret: above zero the whole major line qualifies; at zero the minor (and
  // below that the patch) takes over the stability guarantee instead.
  if (major > 0) return true
  return minor === Number(match[3]) && patch >= Number(match[4])
}

/**
 * Resolve a tracking rule against the registry facts into ONE install target.
 *
 * This is the pure decision the scheduler and the panel share: given what is
 * installed and what is published, does the user's tracking choice name a
 * version worth installing? A `pin` tracks nothing by definition; an unknown
 * installed version resolves to nothing, because "newer" is meaningless
 ...
 * @returns {{ target?: string }} the resolved target, when one exists.
 */
export function resolveTarget(track, installed, published) {
  if (installed === undefined || track.kind === 'pin') return {}
  if (track.kind === 'tag') {
    const candidate = published.distTags[track.tag ?? '']
    if (!isInstallableVersion(candidate)) return {}
    return compareVersions(candidate, installed) > 0 ? { target: candidate } : {}
  }
  let best
  ...
  return compareVersions(best, installed) > 0 ? { target: best } : {}
}
 ...
 *
 * dsh is a monorepo package, but releases are cut on the repository, so only
 * owner and name matter. Accepts the shapes npm manifests actually carry: a
 * URL string or the `{ type, url }` object, with or without the `git+`
 * prefix and `.git` suffix. Anything unparseable yields undefined rather than
 * guessing.
...
/**
 * Fetch the abbreviated packument of the dsh package, trying the configured
 * registry first and falling back to {@link FALLBACK_REGISTRIES} when the
 * read fails at the network layer (DNS, connect, TLS, timeout). A registry
 * that answers with a non-2xx status is reachable and its answer is final —
 * no fallback, the error surfaces as-is.
 * @param {{ registry?: string; fetchImpl?: typeof fetch; timeoutMs?: number }} [deps] - test seams.
 * @returns {Promise<{ distTags: Record<string, string>; versions: string[]; registry: string }>} published facts and the registry that served them.
 */
export async function fetchPublished(deps = {}) {
  const primary = normalizeRegistry(deps.registry ?? DEFAULT_REGISTRY)
  const tried = [primary, ...FALLBACK_REGISTRIES.filter(m => m !== primary)]
  let lastError
  for (const registry of tried) {
    try {
      const facts = await readPackument(registry, deps.fetchImpl ?? fetch, deps.timeoutMs)
      return { ...facts, registry }
    } catch (error) {
      if (error instanceof RegistryHttpError) throw error
      lastError = error
    }
  }
  const detail = lastError instanceof Error ? lastError.message : String(lastError)
  throw new Error(`registry unreachable (tried ${tried.join(', ')}): ${detail}`)
}

/** The registry answered; its status is a real answer, not a network problem. */
class RegistryHttpError extends Error {
  /**
   * @param {number} status - the HTTP status code.
   */
  constructor(status) {
    super(`registry read failed: HTTP ${status}`)
    this.name = 'RegistryHttpError'
  }
}

/**
 * One registry read: request the abbreviated packument and parse it.
 * @param {string} registry - the base URL, already normalized.
 * @param {typeof fetch} fetchImpl - the fetch implementation.
 * @param {number | undefined} timeoutMs - read timeout.
 * @returns {Promise<{ distTags: Record<string, string>; versions: string[] }>} the parsed packument.
 */
async function readPackument(registry, fetchImpl, timeoutMs) {
  const url = `${registry}/${DSH_PACKAGE.replace('/', '%2F')}`
  const response = await fetchImpl(url, {
    headers: { accept: ABBREVIATED },
    signal: AbortSignal.timeout(timeoutMs ?? REGISTRY_TIMEOUT_MS),
  })
