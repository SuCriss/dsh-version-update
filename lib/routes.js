 ...
 *
 * @param {{ updater: { view: () => { state: string; version?: string; log: string; error?: string }; start: (version: string, trigger?: 'manual' | 'auto' | 'scheduled') => object }; restarter?: { restart: () => object; cancelPending: () => void }; running?: string; fence?: (req: import('node:http').IncomingMessage) => boolean; registry?: string; fetchImpl?: typeof fetch; installDir?: string; repoSlug?: string; notes?: (repo: string, version: string) => Promise<{ notes?: string; url?: string }>; ambient?: () => Record<string, unknown>; policy?: { get: () => unknown; set: (next: unknown) => void }; snapshots?: { list: () => unknown[]; restore: (version: string) => Promise<{ ok: boolean; error?: string }> | { ok: boolean; error?: string } }; auto?: (published: { distTags: Record<string, string>; versions: string[] }) => Promise<void> }} deps - the runner, restart, facts, injected operations, and the automation decision.
