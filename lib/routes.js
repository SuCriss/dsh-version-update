...
/**
 * A resolved release-notes lookup wired by the composition when enabled.
 * @typedef {(repo: string, version: string) => Promise<{ notes?: string; url?: string }>} NotesReader
 */

/**
 * What the periodic auto-check last concluded, as the routes expose it.
 * @typedef {{ checkedAt?: number; updateAvailable: boolean; latestVersion?: string; error?: string }} AutoCheckView
 */

/**
 * The derived rollback facts, as the routes expose them.
 * @typedef {{ rollbackTarget?: string; recent: { at: number; from?: string; to: string; result: string }[] }} HistorySummaryView
 */

/**
 * Build the version-update route family.
 * @param {{ updater: { view: () => { state: string; version?: string; log: string; error?: string }; start: (version: string) => object }; restarter?: { restart: () => object }; running?: string; fence?: (req: import('node:http').IncomingMessage) => boolean; registry?: string; fetchImpl?: typeof fetch; installDir?: string; repoSlug?: string; notes?: NotesReader; autoCheck?: () => AutoCheckView; historySummary?: () => HistorySummaryView }} deps - the update runner, the restart runner, the version this process booted with, plus test seams.
...
export function makeRoutes(deps) {
  ...
  /**
   * The ambient facts both polling routes carry: what the periodic auto-check
   * last concluded, and the derived rollback offer. Both are optional wires —
   * a composition without them simply omits the fields.
   * @returns {object} the extra result fields.
   */
  const ambient = () => ({
    ...(deps.autoCheck !== undefined ? { autoCheck: deps.autoCheck() } : {}),
    ...(deps.historySummary !== undefined ? deps.historySummary() : {}),
  })

  // Mounted only when release notes are enabled AND the installed manifest
  // names a GitHub repository — without both there is nothing this route
  // could ever answer with. The guard's narrowing does not survive into the
  // async handler closure, so the checked values are captured first.
