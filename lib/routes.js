 ...
 *
 * @param {{ updater: { view: () => { state: string; version?: string; log: string; error?: string }; start: (version: string, trigger?: 'manual' | 'auto' | 'scheduled') => object }; restarter?: { restart: () => object; cancelPending: () => void }; running?: string; fence?: (req: import('node:http').IncomingMessage) => boolean; registry?: string; fetchImpl?: typeof fetch; served?: (registry: string) => void; installDir?: string; repoSlug?: string; notes?: (repo: string, version: string) => Promise<{ notes?: string; url?: string }>; ambient?: () => Record<string, unknown>; policy?: { get: () => unknown; set: (next: unknown) => void }; snapshots?: { list: () => unknown[]; restore: (version: string) => Promise<{ ok: boolean; error?: string }> | { ok: boolean; error?: string }; remove: (version: string) => Promise<{ ok: boolean; error?: string }> | { ok: boolean; error?: string } }; auto?: (published: { distTags: Record<string, string>; versions: string[] }) => Promise<void> }} deps - the runner, restart, facts, injected operations, and the automation decision.
    ...
    routes: [
      {
        ...
        handler: guarded('GET', async (_req, res) => {
          ...
          // Tell the host which registry these versions came FROM. A read that
          // fell through to a mirror means the configured one is unreachable,
          // and an install that still asks npm for the configured URL would be
          // asking the registry that just failed for the version it is offering.
          if (published !== undefined && deps.served !== undefined) deps.served(published.registry)
          // A successful check doubles as an automation trigger: the freshly
          // read facts go to the scheduler, so an `auto` policy decides and
          // installs (or parks for its window) even when no daily checkAt is
          // configured. A decision failure must never fail the panel's read.
          if (published !== undefined && deps.auto !== undefined) {
            try {
              await deps.auto(published)
            } catch {
              // The panel still gets its facts; the next check re-decides.
            }
          }
          writeJson(res, 200, {
            result: {
