...
`
    ...
    function ConfirmCard(props) {
      const { t, version, installed, onConfirm, onCancel } = props
      const ref = React.useRef(null)
      React.useEffect(() => { ref.current?.focus() }, [])
      return h('section', { className: 'dshvu_card dshvu_confirm', role: 'group', 'aria-label': t('confirm.title') },
        h('h3', { className: 'dshvu_title' }, t('confirm.title')),
        h('p', { className: 'dshvu_confirmBody' }, t('confirm.body', { version, installed: installed ?? t('unknown') })),
        ...
        h('div', { className: 'dshvu_row dshvu_confirmActions' },
          ...
          }, t('confirm.proceed', { version }))))
    }

    /** The version-update settings page. */
    function VersionUpdateSection(props) {
      const { t, useVersionUpdate, check, select, toggleLog, requestUpdate, confirmUpdate, cancelUpdate, restart } = props
        ...
        state.versions.length > 0
          ? h('section', { className: 'dshvu_card' },
            ...
            h('div', { className: 'dshvu_row' },
              ...
              }, running ? t('updating') : state.selected !== undefined && state.selected !== state.installed
