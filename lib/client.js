...
`
    ...
    class VersionUpdateController {
      ...
      offerRestart(installed, running) {
        this.overlay.show({
          ...
          actions: [
            // Same guarantee as the countdown's "Later": hide the offer AND
            // disarm any pending host-side fallback restart, so one word does
            // not mean two different things depending on how the offer arose.
      ...
      cancelCountdown() {
        ...
        // "later" is not overridden when the grace period expires. The empty
        // object is NOT optional: `call` only issues a POST when it is given a
        // body, and the route answers every other method with 405 — which this
        // catch would silently swallow, leaving the host to restart anyway.
      ...
      restart = async (version) => {
        // One restart in flight per page. The countdown expiring and a click on
        // "Restart now" are the same intent arriving twice, and the second POST
        // after the handoff is armed either answers nothing (the host is gone)
        // or re-enters the wait loop on top of the one already running.
    ...
    function VersionUpdateSection(props) {
      const {
        ...
        requestRestore, confirmRestore, cancelRestore, deleteSnapshot,
