...
`
    ...
    class VersionUpdateController {
       ...
       * effective value, which becomes the new form baseline.
       * @param {object} patch - the changed fields.
       */
      savePolicy = async (patch) => {
        if (this.snapshot.savingPolicy === true) return
        this.patch({ savingPolicy: true, policyError: undefined })
        try {
          const result = await call(VERSION_API.policy, patch)
          this.patch({ policy: result.policy, savingPolicy: false })
        } catch (error) {
          this.patch({ policyError: this.describeError(error), savingPolicy: false })
        }
      }
    ...
    function PolicyCard(props) {
      const { t, policy, saving, error, onSave, onRefresh } = props
          ...
          control: h('span', { className: 'dshvu_row' },
            ...
            trackKind === 'tag'
              ? h(Select, {
                ...
                options: [
                  { value: 'latest', label: t('channel.latest', { defaultValue: 'latest' }) },
                  { value: 'next', label: t('channel.next', { defaultValue: 'next' }) },
        ...
        h('section', { className: 'dshvu_card' },
          ...
          h('ul', { className: 'dshvu_list' },
            state.channels.map(channel => h('li', { key: channel.channel, className: 'dshvu_item' },
              h('span', { className: 'dshvu_chanName' }, t(`channel.${channel.channel}`, { defaultValue: channel.channel })),
        ...
        taskLine !== undefined || state.task.log
          ? h('section', { className: 'dshvu_card' },
            ...
            h('div', { className: 'dshvu_row', style: { marginTop: 10 } },
              ...
              state.task.state === 'running' ? h('span', { className: 'dshvu_spin' }, t('task.following')) : null,
          ...
          ? h('section', { className: 'dshvu_card' },
            ...
            h('ul', { className: 'dshvu_list' },
              state.history.map((entry, index) => h('li', { key: `${entry.at}-${index}`, className: 'dshvu_item' },
                ...
                h('span', { className: 'dshvu_itemMain' },
                  `${fmtTime(entry.at)}${entry.from !== undefined ? ` · ${entry.from} → ${entry.to}` : ''}${typeof entry.trigger === 'string' ? ` · ${t(`history.trigger.${entry.trigger}`, { defaultValue: entry.trigger })}` : ''}${entry.restored === true ? ` · ${t('history.restored')}` : ''}`),
                h('span', {
                ...
                }, t(`history.result.${entry.result}`, { defaultValue: entry.result }))))))
          : null,
    ...
    const zh = {
      ...
      installDowngrade: '降级',
      installDowngradeTo: '降级到 {version}',
      badge: { current: '当前', ahead: '有更新' },
      channel: { latest: 'latest 稳定通道', next: 'next 预发布通道' },
      versions: { title: '可用版本' },
      confirm: {
        title: '确认安装',
        downgradeTitle: '确认降级',
        body: '即将把 @deepseek-ai/dsh 安装到 {version}（当前 {installed}）。安装前会自动为本机当前版本创建回滚快照。',
        downgradeBody: '即将把 @deepseek-ai/dsh 降级到 {version}（当前 {installed}）。安装前会自动创建当前版本的回滚快照。',
        impact: '该操作会改写本机全局 npm 包；完成后将重启 dsh 宿主进程并重新载入页面。',
        proceed: '安装 {version}',
        proceedDowngrade: '降级到 {version}',
        cancel: '取消',
      },
      restoreConfirm: {
        title: '确认恢复快照',
        body: '将从本地快照把 @deepseek-ai/dsh 恢复到 {version}。恢复不联网、通常数秒完成。',
        impact: '恢复会直接覆盖磁盘上的现有安装；完成后同样需要重启宿主进程才能生效。',
        proceed: '恢复到 {version}',
      },
      policy: {
        title: '更新策略',
        ...
        save: '保存策略',
        saving: '保存中…',
        nextCheck: '下次自动检查 {time}',
        mode: { label: '自动模式' },
        'mode.off': '关闭',
        'mode.notify': '仅提醒',
        'mode.auto': '静默自动更新',
        'mode.hint.off': '只在面板里显示可用更新，不做任何自动动作。',
        'mode.hint.notify': '发现新版本时在面板显著提示，仍需手动确认安装。',
        'mode.hint.auto': '发现新版本后自动安装并按下方设置处理重启，无需人工确认。',
        track: { label: '跟踪目标' },
        'track.tag': '跟随 dist-tag',
        'track.line': '跟随版本线',
        'track.pin': '固定当前',
        'track.hint.tag': '始终跟随所选 dist-tag 指向的最新发布。',
        'track.hint.line': '只接受 ^ 或 ~ 版本线内的稳定版（如 ^0.4.0 表示 0.4.x 的最新版）。',
        'track.hint.pin': '不跟踪任何目标；只有手动选择才会改变版本。',
        customTag: '自定义…',
        tagPlaceholder: '输入 dist-tag',
        restart: { label: '安装完成后' },
        'restart.ask': '询问我是否重启（倒计时 20 秒）',
        'restart.auto': '自动重启宿主（无人值守）',
        'restart.hint.ask': '安装结束后弹出可取消的重启倒计时。',
        'restart.hint.auto': '静默更新完成后约 10 秒自动重启宿主进程。',
        window: { label: '执行时间窗' },
        'window.hint': '仅在「静默自动更新」模式下生效：只允许在该本地时间段内开始自动安装。留空表示任何时间。起止相同表示全天；结束早于起始表示跨午夜。',
        checkAt: { label: '每日定时检查' },
        'checkAt.hint': '每天在此时刻后台检查一次更新。留空表示不定时检查。',
      },
      task: {
        title: '安装任务',
      ...
      snapshots: {
        title: '快照与回滚',
        hint: '每次安装前会自动备份当前版本。从快照恢复不依赖网络，通常数秒完成。',
        empty: '还没有快照。首次安装或手动更新后这里会出现可回滚的版本。',
        restore: '恢复此版本',
        unusable: '快照不可用',
      },
      history: {
        title: '最近活动',
        restored: '快照恢复',
        trigger: { manual: '手动', auto: '自动', scheduled: '计划' },
        result: { ok: '成功', failed: '失败' },
      },
      lastCheck: {
        at: '上次检查 {time}',
        ahead: '有可用更新',
      ...
      restart: {
        title: '重启 dsh 宿主',
        staleBody: '磁盘上已是 {installed}，而运行中的进程仍是 {running}。重启后新版本生效，页面资源也会随之更新。',
        unavailable: '需要重启（磁盘 {installed}/运行 {running}），但当前环境不支持自动重启，请在终端手动操作。',
        countdown: '将在 {seconds} 秒后重启到 {version}。重启会结束当前所有会话与后台任务。',
        pending: '正在交接端口并退出旧进程…',
        waiting: '旧进程已退出，正在等待新进程就绪（{version}）…',
        reload: '{version} 已就绪，正在重新载入页面…',
        timeout: '等待新进程超时（{version}）。请检查终端，或重新运行 dsh web。',
        failed: '自动重启失败：{error}。请自行停止并启动 dsh web。',
        now: '立即重启',
        later: '稍后',
        dismiss: '知道了',
        reloadNow: '仍然刷新',
    ...
    const en = {
      ...
      installDowngrade: 'Downgrade',
      installDowngradeTo: 'Downgrade to {version}',
      badge: { current: 'current', ahead: 'update' },
      channel: { latest: 'latest (stable)', next: 'next (pre-release)' },
      versions: { title: 'Available versions' },
      confirm: {
        title: 'Confirm install',
        downgradeTitle: 'Confirm downgrade',
        body: 'This installs @deepseek-ai/dsh {version} (currently {installed}). A rollback snapshot of the current version is taken automatically first.',
        downgradeBody: 'This downgrades @deepseek-ai/dsh to {version} (currently {installed}). A rollback snapshot is taken automatically first.',
        impact: 'The operation rewrites this machine\'s global npm package; afterwards the dsh host process restarts and this page reloads.',
        proceed: 'Install {version}',
        proceedDowngrade: 'Downgrade to {version}',
        cancel: 'Cancel',
      },
      restoreConfirm: {
        title: 'Confirm snapshot restore',
        body: 'This restores @deepseek-ai/dsh to {version} from the local snapshot. No network needed; usually a matter of seconds.',
        impact: 'Restoring overwrites the installation currently on disk; a host restart is required afterwards.',
        proceed: 'Restore {version}',
      },
      policy: {
        title: 'Update policy',
        ...
        mode: { label: 'Automation' },
        'mode.off': 'Off',
        'mode.notify': 'Notify only',
        'mode.auto': 'Silent auto-update',
        'mode.hint.off': 'Only surface updates in the panel; nothing runs on its own.',
        'mode.hint.notify': 'Highlight discoveries in the panel; installs stay manual.',
        'mode.hint.auto': 'Found versions install automatically and restart per the setting below.',
        track: { label: 'Tracking' },
        'track.tag': 'Follow dist-tag',
        'track.line': 'Follow a version line',
        'track.pin': 'Pinned',
        'track.hint.tag': 'Always follow what the chosen dist-tag points at.',
        'track.hint.line': 'Accept only stable releases within a caret/tilde line (e.g. ^0.4.0 — newest 0.4.x).',
        'track.hint.pin': 'Track nothing; only an explicit choice changes the version.',
        customTag: 'Custom…',
        tagPlaceholder: 'dist-tag',
        restart: { label: 'After install' },
        'restart.ask': 'Ask me to restart (20 s countdown)',
        'restart.auto': 'Restart automatically (unattended)',
        'restart.hint.ask': 'A cancellable countdown appears when an install settles.',
        'restart.hint.auto': 'Silent installs restart the host after roughly ten seconds.',
        window: { label: 'Execution window' },
        'window.hint': 'Applies to silent auto-update: automatic installs may only START inside this local time span. Empty means anytime. Equal bounds mean all day; end before start wraps past midnight.',
        checkAt: { label: 'Daily check at' },
        'checkAt.hint': 'Run one background check every day at this time. Empty disables the schedule.',
      },
      task: {
        title: 'Install task',
        running: 'Installing {version}…',
        done: '{version} installed.',
        failed: 'Install failed: {error}',
        showLog: 'Show install log',
        hideLog: 'Hide log',
        following: 'following output…',
      },
      snapshots: {
        title: 'Snapshots & rollback',
        hint: 'Every install backs up the previous version first. Restoring a snapshot needs no network and takes seconds.',
        empty: 'No snapshots yet. One appears here after the first install or manual update.',
        restore: 'Restore',
        unusable: 'unusable',
      },
      history: {
        ...
        trigger: { manual: 'manual', auto: 'auto', scheduled: 'scheduled' },
        result: { ok: 'ok', failed: 'failed' },
      },
      lastCheck: {
        at: 'Last check {time}',
        ahead: 'update available',
        current: 'up to date',
      ...
      restart: {
        title: 'Restart the dsh host',
        staleBody: 'Disk holds {installed} while the running process is {running}. Restarting activates the new version and refreshes this page\'s assets.',
        unavailable: 'A restart is required (disk {installed}/running {running}), but this environment cannot restart automatically — please do it from a terminal.',
        countdown: 'Restarting into {version} in {seconds}s. This ends every session and background job on the host.',
        pending: 'Handing the port over and exiting the old process…',
        waiting: 'Old process exited; waiting for the replacement ({version})…',
        reload: '{version} is ready — reloading…',
        timeout: 'Timed out waiting for the replacement ({version}). Check the terminal, or run dsh web again.',
        failed: 'Automatic restart failed: {error}. Stop and start dsh web yourself.',
        now: 'Restart now',
        later: 'Later',
        dismiss: 'Got it',
        reloadNow: 'Reload anyway',
        pendingShort: 'Restarting…',
      },
