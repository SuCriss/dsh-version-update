...
export const VERSION_API = {
  ...
  /**
   * Disarm a pending host-side fallback restart. The panel calls this when
   * the user picks "later" on an interactive restart offer or countdown, so
   * the unattended grace period armed by `onSettled` does not restart the
   * host out from under a live page that explicitly deferred.
   */
