...
export function createRestarter(deps = {}) {
  ...
  return {
    ...
    restart() {
      ...
      // Auto-rollback is armed per restart, not per install: what the helper
      // needs is the version this process is running RIGHT NOW (the thing to
      // fall back to if the replacement never becomes ready), plus the same
      // npm resolution and registry the updater installs with.
      const rollback = deps.rollback?.()
